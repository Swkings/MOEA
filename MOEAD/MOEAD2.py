...
class MOEAD:
	...
	def Tchebycheff(self, Y, Lambda, Z):
		...
		:return:
		"""
		Lambda = [0.00001 if item==0 else item for item in Lambda]
		tchebycheff = np.max((Y - Z) * Lambda)
		return tchebycheff
	...
	def getPopulationY(self):
		Y = np.empty((self.populationSize, self.objFuncNum))
		for i in range(self.populationSize):
			Y[i] = self.getFuncValue(self.population[i])
		return Y
...
if __name__ == '__main__':
	...
	MOEA = MOEAD(ObjFunc=DTLZ, stepLength=15, T=20, maxGeneration=500)
	print("populationSize:", MOEA.populationSize)
	MOEA.run()
	populationY = MOEA.getPopulationY()
	X, Y, Z = populationY[:, 0], populationY[:, 1], populationY[:, 2]
	...
	# MOEA = MOEAD(ObjFunc=ZDT, stepLength=299, T=20, maxGeneration=100)
	# print("populationSize:", MOEA.populationSize)
	# MOEA.run()
	# populationY = MOEA.getPopulationY()
	# X, Y, = populationY[:, 0], populationY[:, 1]
